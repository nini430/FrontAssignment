...
export const createNewUser=asyncHandler(async(req:Request<{},{},UserInput>,res:Response)=>{
...
export const updateUser=asyncHandler(async(req:Request<{id:string},{},UserInput>,res:Response,next:NextFunction)=>{
    let users=getAllUsers();
    const userLocation=findUserLocation(+req.params.id);
    if(userLocation===-1) {
        return next(new ErrorResponse('User Not Found',404));
    }
    users=users.map((user:UserInput)=>user.id===+req.params.id?req.body:user);
    saveUsers(users);
